...
class HomeSection extends Section {
  ...
  @property()
  content: (that: this) => any = (that: this) =>
    (this.appState.view.map as WebScene).portalItem.snippet;
  ...
  render() {
    const timetable = this.timetable ? (
      <section class="Hours">
        <h2 class="slash-title">Credits</h2>
        <h4>Yannik Messerli</h4>
        <h5>Software Developer at Esri R&D Zurich</h5>
        <h4>Edward Wong</h4>
        <h5>Junior GIS Developer at Eagle Technology New Zealand</h5>
        <div>{this.timetable.render()}</div>
      </section>
    ) : null;
    ...
    watchUtils.whenOnce(this, "appState", () => {
      watchUtils.on(this, "appState.view.map.layers", "change", () => {
        if (this.appState && this.appState.view.map.layers.length > 0) {
          this.infoPointsLayer = this.appState.view.map.layers.find(
            (layer) =>
              layer.title.indexOf(appUtils.EXTERNAL_INFOPOINT_LAYER_PREFIX) > -1
          ) as FeatureLayer;
