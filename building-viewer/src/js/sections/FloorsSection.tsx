...
class LegendWrapper extends Widget {
  ...
  render() {
    return (
      <div class={this.classes({ hide: this.hide })}>
        {this.legend.render()}
      </div>
    );
  }
...
class PlayButton extends Widget {
  ...
  render() {
    ...
    return (
      <button
        class={this.classes(dynamicCss, "play_button")}
...
export class Floor extends Widget {
  ...
  render() {
    const audio = this.audio ? (
      <p>Listen to the name of this floor {this.playButton.render()}</p>
    ) : null;
    return (
      <div>
        {this.content(this)}
...
export class FloorsSection extends Section {
  ...
  render() {
    function getName(level: number) {
      if (level === 0) return "A";
      else if (level === 1) return "B";
      else if (level === 2) return "C";
      else if (level === 3) return "D";
      else return "Block ?";
    }
    const currentLevel = this.floors
      ? this.floors.getItemAt(this.selectedFloor)
      : null;
    ...
    return currentLevel ? (
      <div id={this.id} bind={this} key={this}>
        <div class="level">Block</div>
        <h1 class="number">{getName(this.selectedFloor)}</h1>
        {title}
  ...
  paneRight() {
    const floorSelector = this.floorSelector
      ? this.floorSelector.render()
      : null;
  ...
  postInitialize() {
    watchUtils.whenOnce(this, "appState", () => {
      ...
      const floorSelectorCtorArgs =
        this.minFloor != null && this.maxFloor != null
          ? {
              appState: this.appState,
      ...
      watchUtils.on(
        this,
        "appState.view.map.layers",
        "change",
        this.getExtraInfoLayers.bind(this)
      );
      ...
      watchUtils.init(this, "selectedFloor", (selectedFloor) => {
        ...
        if (this.picturePointsLayer) {
          this.picturePointsLayer.definitionExpression =
  ...
  onEnter() {
    ...
    this.appState.view.environment.lighting.date = new Date(
      "Thu Aug 01 2019 03:00:00 GMT+0200 (Central European Summer Time)"
    );
    this.handles.add(
      this.appState.view.on("click", (event: any) => {
        // the hitTest() checks to see if any graphics in the view
        // intersect the given screen x, y coordinates
        this.appState.view.hitTest(event).then((response) => {
  ...
  private getExtraInfoLayers() {
    if (this.appState && this.appState.view.map.layers.length > 0) {
      ...
      if (!this.layer) {
        this.layer = appUtils.findLayer(
          this.appState.view.map.layers,
          this.layerNameForInfoPoint
        ) as FeatureLayer;
        if (this.layer) {
          this.layer.visible = false;
      ...
      // Get extra pictures:
      if (!this.picturePointsLayer) {
        this.picturePointsLayer = appUtils.findLayer(
          this.appState.view.map.layers,
          this.layerNameForPicturePoint
        ) as FeatureLayer;
        if (this.picturePointsLayer) {
          this.picturePointsLayer.visible = false;
