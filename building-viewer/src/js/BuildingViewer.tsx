...
class BuildingViewer extends Widget {
  ...
  constructor(args: BuildingViewerCtorArgs) {
    ...
    this.view = appUtils.createViewFromWebScene({
      websceneId: args.websceneId,
  ...
  initialize() {
    ...
    (this.view.map as WebScene).when(() => {
      ...
      promiseUtils
        .eachAlways(
          this.view.map.layers.map((l) => this.appState.view.whenLayerView(l))
        )
        .then(() => {
          ///////////////////////////////////
          // Main building to present:
          const BSL = this.appState.view.map.layers.find(
            (layer) => layer.title.indexOf(appUtils.MAIN_LAYER_PREFIX) > -1
          );

          if (!BSL) {
            throw new Error(
              "Cannot find the main BuildingSceneLayer (" +
                appUtils.MAIN_LAYER_PREFIX +
                ") in the webscene " +
                this.websceneId
            );
          }
          ...
          const surroundingsLayer = this.appState.view.map.layers.find(
            (layer) =>
              layer.title
                .toLowerCase()
                .indexOf(appUtils.CITY_LAYER_PREFIX.toLowerCase()) > -1
          ) as SceneLayer;
      ...
      this.sections.forEach((section) => {
        const slide = (this.view.map as WebScene).presentation.slides.find(
          (slide) => slide.title.text === section.title
        );
        if (slide) {
  ...
  postInitialize() {
    this.own(
      this.sections.on("go-to", (camera: Camera) => {
