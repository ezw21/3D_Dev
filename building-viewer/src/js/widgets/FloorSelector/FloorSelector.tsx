...
class FloorSelector extends Widget {
  ...
  render() {
    ...
    const levels = Array.from(
      Array(Math.abs(this.minFloor) + this.maxFloor + 1).keys()
    )
      .reverse()
      .map((rawLevel: number) => {
        ...
        return (
          <li
            class={this.classes("level", activeClass)}
            onclick={() => this.activeLevel(level)}
          >
            {getName(level)}
          </li>
        );
    ...
    return (
      <div bind={this} key={this} class="floor-selector">
